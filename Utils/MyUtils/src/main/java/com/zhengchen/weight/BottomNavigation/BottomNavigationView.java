package com.zhengchen.weight.BottomNavigation;
...
public class BottomNavigationView extends RelativeLayout {
    ...
    //文字颜色
    private int colorActive=R.color.colorActive;
    private int colorInactive=R.color.colorInactive;
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            colorActive = array.getColor(R.styleable.BottomNavigationView_colorActive, -1);
            colorInactive = array.getColor(R.styleable.BottomNavigationView_colorInactive, -1);
            array.recycle();
        }
    }
...
//            throw new NullPointerException("You need at least one item");
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            title.setTextColor(colorInactive);
            viewList.add(view);
...
//                icon.setColorFilter(i == currentItem ? itemActiveColorWithoutColoredBackground : itemInactiveColor);
    ...
    private void onBottomNavigationItemClick(final int itemIndex) {
        ...
        if (currentItem == itemIndex||noshowposition==itemIndex) {
        ...
        for (int i = 0; i < viewList.size(); i++) {
            if (i == itemIndex) {
                ...
                BottomNavigationUtils.changeTextColor(title, colorInactive, colorActive);
...
//                    BottomNavigationUtils.changeImageColorFilter(icon, itemActiveColorWithoutColoredBackground, itemInactiveColor);
                ...
                BottomNavigationUtils.changeTextColor(title, colorActive, colorInactive);
    ...
    private int noshowposition=-1;

    /**
     * 设置不显示该页面
     */
    public void setNoShow(int position){
        noshowposition=position;
    }
    ...
    public void setUpWithViewPager(ViewPager pager, int[] colorResources, int[] imageResources, int[] imageResources2) {
        this.mViewPager = pager;
        if (pager.getAdapter().getCount() != colorResources.length || pager.getAdapter().getCount() != imageResources.length|| pager.getAdapter().getCount() != imageResources2.length)
        ...
        for (int i = 0; i < pager.getAdapter().getCount(); i++)
            addTab(new BottomNavigationItem(pager.getAdapter().getPageTitle(i).toString(), colorResources[i], imageResources[i],imageResources2[i]));
    }
