...
public final class MyARouter {
    ...
    private volatile static MyARouter instance = null;
    ...
    public static void init(Application application) {
        if (!hasInit) {
            ...
            _ARouter.logger.info(Consts.TAG, "MyARouter init start.");
            ...
            _ARouter.logger.info(Consts.TAG, "MyARouter init over.");
        }
    }
     ...
     */
    public static MyARouter getInstance() {
        if (!hasInit) {
            throw new InitException("MyARouter::Init::Invoke init(context) first!");
        } else {
            if (instance == null) {
                synchronized (MyARouter.class) {
                    if (instance == null) {
                        instance = new MyARouter();
                    }
                }
            }
            return instance;
    ...
    /**
     * The interface is not stable enough, use 'MyARouter.inject();';
     */
    @Deprecated
