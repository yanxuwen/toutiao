...
final class _ARouter {
    ...
    protected static synchronized boolean init(Application application) {
        ...
        logger.info(Consts.TAG, "MyARouter init success!");
        hasInit = true;
    ...
    static synchronized void destroy() {
        if (debuggable()) {
            ...
            logger.info(Consts.TAG, "MyARouter destroy success!");
        } else {
    ...
    static synchronized void openDebug() {
        debuggable = true;
        logger.info(Consts.TAG, "MyARouter openDebug");
    }

    static synchronized void openLog() {
        logger.showLog(true);
        logger.info(Consts.TAG, "MyARouter openLog");
    }
    ...
    static void attachBaseContext() {
        Log.i(Consts.TAG, "MyARouter start attachBaseContext");
        try {
            ...
            Log.i(Consts.TAG, "MyARouter hook instrumentation success!");
        } catch (Exception ex) {
            Log.e(Consts.TAG, "MyARouter hook instrumentation failed! [" + ex.getMessage() + "]");
        }
    }

    static synchronized void printStackTrace() {
        logger.showStackTrace(true);
        logger.info(Consts.TAG, "MyARouter printStackTrace");
    }
    ...
    static synchronized void monitorMode() {
        monitorMode = true;
        logger.info(Consts.TAG, "MyARouter monitorMode on");
    }
    ...
    static void inject(Object thiz) {
        AutowiredService autowiredService = ((AutowiredService) MyARouter.getInstance().build("/arouter/service/autowired").navigation());
    ...
    protected Postcard build(String path) {
        if (TextUtils.isEmpty(path)) {
            ...
            PathReplaceService pService = MyARouter.getInstance().navigation(PathReplaceService.class);
            if (null != pService) {
    ...
    static void afterInit() {
        ...
        Object sdd = MyARouter.getInstance().build("/myarouter/service/interceptor").navigation();
        interceptorService = (InterceptorService)sdd;
    }
    ...
    protected Object navigation(final Context context, final Postcard postcard, final int requestCode, final NavigationCallback callback) {
        try {
            ...
            if (null != callback) {
                ...
                DegradeService degradeService = MyARouter.getInstance().navigation(DegradeService.class);
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    ...
                    public void run() {
                        if (requestCode > 0) {  // Need start for result
                            IntentUtils.getInstance().startActivityForResult((Activity) currentContext, intent, requestCode, postcard.getOptionsBundle());

                        } else {
                            IntentUtils.getInstance().startActivity(currentContext, intent, postcard.getOptionsBundle());
                        }
                        ((Activity)currentContext).overridePendingTransition(R.anim.common_open_enter, 0);
