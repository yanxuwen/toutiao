...
@Route(path = ARouterPath.Module_Bottomtab0_BottomTab0Fragment)
class BottomTab0Fragment : MyFragment(), View.OnClickListener, ViewPager.OnPageChangeListener {
    var list_category: List<Category.DataBeanX.DataBean>?=null

    public val viewpager:ViewPager by lazy {
        flayout?.viewPager!!
    }
    private val dm: DisplayMetrics by lazy {
        activity.resources.displayMetrics
    }
    public  var mMPagerAdapter: MStatePagerAdapter?=null
    private val list_title = ArrayList<String>()
    override fun getMainView(): View? {
        if (mainview == null) {
            mainview = inflater.inflate(R.layout.bt0_, null)
        }
        return mainview
    }
    ...
    override fun initView() {
        ...
        if(list_category!=null&& !list_category?.isEmpty()!!){
            ...
            if (list_title.isEmpty()) {
                list_title.addAll(context.resources.getStringArray(R.array.bt0_channel).toList())
            }
        }
        initViewPage()
...
//        }, 100)
    ...
    override fun init() {
        ...
        var currentItem = flayout?.viewPager?.currentItem ?: 0
        if (mMPagerAdapter != null && mMPagerAdapter?.count?:0 > currentItem) {
    ...
    fun initMagicIndicator() {
        ...
        commonNavigator.adapter = object : CommonNavigatorAdapter() {
            ...
            override fun getTitleView(context: Context, index: Int): IPagerTitleView {
                ...
                clipPagerTitleView.clipColor = context.resources.getColor(R.color.bt0_tab)
...
//        flayout?.appBarLayout?.setExpanded(true)
        var category = if(list_category!=null&&!list_category?.isEmpty()!!){
            ...
            context.resources.getStringArray(R.array.bt0_channel_code)[position]

        }
        (mMPagerAdapter?.getItem(position) as TopTabRecommendFragment).category= category
        (mMPagerAdapter?.getItem(position) as TopTabRecommendFragment).isSoonVideo= (category=="hotsoon_video")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        (mMPagerAdapter?.getItem(flayout?.viewPager?.currentItem ?: 0) as MyFragment).onActivityResult(requestCode, resultCode, data)
    }
